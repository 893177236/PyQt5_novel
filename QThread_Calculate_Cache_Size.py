...
class QThread_Calculate_Cache_Size(QThread):
        ...
        for root, dirs, files in cache_images:
            for file in files:
                if cache_path + f"/{file}" in useful_pic:
                    log.info(f"匹配到有用的图片:{file}")
                    useful_pic_size += os.path.getsize(filename=cache_path + f"/{file}")
                    useful_pic_num += 1
                else:
                    useless_pic_size += os.path.getsize(filename=cache_path + f"/{file}")
                    useless_pic_num += 1
        self.breakSignal_setLabel.emit(
            f"<font color=\"orange\">{useless_pic_num}</font> "
            f"Useless Pictures(<font color=\"red\">{self.calculate_file_size(Byte=useless_pic_size)}</font>)",
